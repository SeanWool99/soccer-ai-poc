...
# why a hard veto is wrong at 93% accuracy.
...
class TeamColourClassifier:
    ...
    def predict_tracklet(self, crops: Sequence[np.ndarray]) -> Optional[int]:
        ...
        This is the call worth using. Every crop here is the same player by
        construction, so voting converts ~85% per-crop accuracy into ~93% per
        player — and a tracklet is also the unit the stitcher reasons about.
        """
...
def team_penalty(team_a: Optional[int], team_b: Optional[int]) -> float:
    ...
    A penalty rather than a veto, deliberately. At 93% accuracy a hard rule
    would forbid roughly one correct link in fourteen, and a forbidden correct
    link is invisible — it just leaves a player fragmented — whereas a penalty
    lets strong motion evidence overrule a doubtful colour call.
    """
